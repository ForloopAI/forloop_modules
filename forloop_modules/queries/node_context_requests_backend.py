...
def update_last_active_dataframe_node_uid(
...
def get_variable_by_name(variable_name: str) -> Response:
    ...
    url = (
        f"{BASE_API}/variables?name={variable_name}&pipeline_job_uid={pipeline_job_uid}"
    )
...
def delete_all_initial_variables() -> Response:
    ...
    response = http_client.delete(
        f"{BASE_API}/pipelines/{pipeline_uid}/initial_variables"
    )
    response.raise_for_status()
    return response
...
def update_initial_variable_by_uid(
    ...
    response = http_client.put(
        f"{BASE_API}/initial_variables/{variable_uid}", json=payload
    )
    response.raise_for_status()
    return response
...
def upload_urls_from_file(path: str) -> Response:
    ...
    response = http_client.post(
        SERVER + ":" + str(PORT) + "/api/v1/upload_urls_from_file", files=file
    )
...
def delete_all_files():
    ...
    flog.info(payload)
    response = http_client.delete(
        SERVER + ":" + str(PORT) + "/api/v1/files", data=payload
    )
    return response
...
def update_last_active_script(script_uid: Optional[str] = None) -> Response:
    payload = {"project_uid": aet.project_uid, "uid": script_uid}
...
def run_pipeline_to_code_conversion() -> Response:
    payload = {"pipeline_uid": aet.active_pipeline_uid, "project_uid": aet.project_uid}
...
def run_inspect_node_code(node_uid: str) -> Response:
    payload = {"uid": node_uid, "project_uid": aet.project_uid}
...
def get_all_databases_by_project_uid() -> Response:
    ...
    project_databases = [
        database for database in databases if database["project_uid"] == aet.project_uid
    ]
...
def store_df_to_google_sheet(dataset_uid: str, sheet_name: str, email: str) -> Response:
    payload = {"dataset_uid": dataset_uid, "sheet_name": sheet_name, "email": email}
...
def get_next_node_predictions(
    initial_node_uid: Optional[str] = None, is_used_for_autopilot: bool = False
) -> Response:
    payload = {
...
def confirm_selected_node_prediction(
    node_uid: str, is_used_for_autopilot: bool = False
) -> Response:
    payload = {"node_uid": node_uid, "is_used_for_autopilot": is_used_for_autopilot}
...
def process_api_adjustments(adjustments_dict: dict, project_uid: str) -> Response:
    payload = {"adjustments_dict": adjustments_dict, "project_uid": project_uid}
...
def filter_webpage_elements_based_on_objective(
    elements: list[dict], objective: str
) -> Response:
    payload = {"elements": elements, "objective": objective}
